...
- **不会背着你回收。** 槽位会一直保有它的端口，直到你释放它，无论它的服务器停了多久。`port-keeper gc` 会列出看起来已被弃用的槽位（在 `stale_days` 内没有任何监听，或者工作副本已经不存在），并且只有加上 `--yes` 才会释放。一个停了一个月的开发服务器，重新启动时不应该发现自己的端口已经被交给了另一个槽位。
