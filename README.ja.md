...
port-keeperを作る前に、既存のツールを調べました。見つかったものは、いくつかの種類に分かれます。ポートをホスト名の裏に隠すローカルプロキシ（常駐が必要です）。ポートの管理が、セッションやロックやメッセージングと並ぶ機能の1つになっている、エージェント調停の基盤。開発サーバーの起動そのものを肩代わりしようとするラッパー。誰がどのポートを持っているかを記録しない、空きポート探し。並列の作業コピーも`.env`も扱わない、エージェント向けのポート登録簿。それぞれの得意分野では優れたものもあります。しかし、「台帳であり、台帳でしかないもの」はありませんでした。

port-keeperは、上に書いた問題を解く、いちばん単純な道具です。
...
- **黙って回収しない。** スロットは、あなたが解放するまでポートを保ちます。サーバーが何日止まっていても同じです。`port-keeper gc`は、放置されたように見えるスロット（`stale_days`の間LISTENが無い、または作業コピーが消えた）を一覧し、`--yes`を付けたときだけ解放します。1か月止めていた開発サーバーが、再開したときに別のスロットへ番号を渡されていた、という事故を防ぐためです。
