...
状態（2026-09-18）: `v0.1.0`と`v0.2.0`を公開済みで、この文書の手順は`release.yml`を実際のタグで二度通している。手順に食い違いが見つかったら、この文書を直すこと。
...
状態（2026-09-18）: 登録済み。Glama上のリリース`0.1.0`と`0.2.0`を公開し、READMEにスコアのバッジを載せた。この節の手順は、port-keeperで二度通した（`0.2.0`ではCreate Releaseの版の既定値が`0.1.1`だったので`0.2.0`に直した。8.3の手順3のとおり）。
