...
- [ ] 7.2 Glamaに登録する。Chromeの操作はClaude Codeが行い、ログインと所有の申告はユーザーが行う。自動リリースの設定はオフにする。確認: Glamaのチェックが`success`になり、Glama上のリリース`0.1.0`が`latest`と表示される
- [ ] 7.3 READMEの3言語にGlamaのバッジを足す。確認: 3ファイルが同じcommitで更新されている
